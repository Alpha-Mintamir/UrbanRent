...
const AccountNav = () => {
  ...
  // Get the subpage from the URL - handle both /account/X and /broker/X paths
  let subpage = isBrokerMode ? pathname.split('/')?.[2] : pathname.split('/')?.[2];
  ...
  return (
    <nav className="mb-8 mt-24 flex w-full flex-col justify-center gap-2 p-8 md:flex-row md:p-0">
      <Link className={linkClases('profile')} to={isBrokerMode ? '/broker/dashboard' : '/account'}>
        <svg
