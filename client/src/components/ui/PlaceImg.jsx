import React from 'react';

const PlaceImg = ({ place, index = 0, className = null }) => {
  // Check if photos exist in the expected format
  if (!className) {
    className = 'object-cover w-full h-full';
  }
  
  // Handle different photo data structures
  if (place.photos && Array.isArray(place.photos) && place.photos.length > 0) {
    // If photos is an array of objects with url property (from Sequelize)
    if (typeof place.photos[0] === 'object' && place.photos[0].url) {
      return <img src={place.photos[index]?.url || ''} alt={place.property_name || 'Property'} className={className} />;
    }
    // If photos is an array of strings (direct URLs)
    if (typeof place.photos[0] === 'string') {
      return <img src={place.photos[index] || ''} alt={place.property_name || 'Property'} className={className} />;
    }
  }
  
  // Fallback to placeholder image if no photos are available
  return <div className={`${className} bg-gray-300 flex items-center justify-center`}>
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-12 h-12 text-gray-500">
      <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 001.5-1.5V6a1.5 1.5 0 00-1.5-1.5H3.75A1.5 1.5 0 002.25 6v12a1.5 1.5 0 001.5 1.5zm10.5-11.25h.008v.008h-.008V8.25zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" />
    </svg>
  </div>;
};

export default PlaceImg;
